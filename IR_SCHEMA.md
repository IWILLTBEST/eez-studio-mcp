...
单文件依旧合法；大工程可按"改文件的人是谁"拆分，清单 `<include>` 缝合（相对路径、环检测、screen/widget/text 重名报错、`<project>` 只许一份）。**清单按示例名命名**（i18n.uixml、motor.uixml…编辑器页签跨示例可区分）；`tools/split_uixml.py` 一键拆分并自检（清单重解析必须与单文件 IR 逐字段一致）：

```text
examples/i18n/
├── i18n.uixml         ← 清单：<include> logic / strings / screens（编译入口，按示例名命名）
├── logic.uixml        ← 逻辑平面：<project> + <var> + <action>（固件工程师的接口，生成 action.h）
├── strings.uixml      ← 文案平面：翻译者只碰这里（无 tr 的工程省略）
├── widgets/           ← 用户部件平面：跨屏复用组件逐件一文件（如 motor 的 StatusBar）
└── screens/main.uixml ← UI 平面：界面 + UI 内部编排（切屏/动画），设计者/AI 迭代，一屏一文件
```

拆分发生在 uixml 解析层（合并成单一 IR），编译器零改动；全部示例均已拆分形态入库，金标准 0.0% 自证无损。
